var SITE_COUPON = function(){
  ...
  const isAllowedCouponIconSrc = (src) => {
    try {
      const url = new URL(src, window.location.origin);
      return url.protocol === 'https:'
        && /(^|\.)imweb\.[a-z.]+$/i.test(url.hostname)
        && /\/images\/site\/download-icon2\.svg$/i.test(url.pathname);
    } catch (e) {
      return false;
    }
  };

  const renderTotalCouponWrap = (wrap, htmlMsg, retryable) => {
    if (!wrap) return;
    const parsed = htmlMsg ? new DOMParser().parseFromString(String(htmlMsg), 'text/html') : null;
    const text = parsed ? (parsed.body.textContent || '').trim() : '';
    if (!text) return;
    wrap.innerHTML = '';
    const btn = document.createElement('button');
    btn.className = 'total-coupon-download tw-w-full tw-py-[15px] tw-flex tw-items-center tw-justify-center btn-coupon tw-font-apple-sd-gothic tw-text-[15px]';
    btn.textContent = text;
    btn.disabled = !retryable;
    if (retryable) {
      btn.id = 'total_coupon_download';
      btn.addEventListener('click', downloadEntireCoupon);
      const imgs = parsed.querySelectorAll('img');
      for (const serverImg of imgs) {
        const src = serverImg.getAttribute('src') || '';
        if (isAllowedCouponIconSrc(src)) {
          const icon = document.createElement('img');
          icon.src = src;
          icon.alt = 'download_icon2';
          icon.className = 'tw-ml-[10px]';
  ...
  let downloadEntireCouponInFlight = false;

  // 상세페이지 전체쿠폰 다운로드
  const downloadEntireCoupon = () => {
    if (downloadEntireCouponInFlight) return;
    ...
    if (downloadReadyCouponList.length > 0) {
      downloadEntireCouponInFlight = true;
      $.ajax({
        type: "POST",
        ...
        success: function (res) {
          ...
          res.map((res) => {
            ...
            if (res.download === "SUCCESS") {
              downloadedCouponCodes.push(res.coupon_code);
          ...
          if (downloadedCouponCodes.length > 0) {
            window.dispatchEvent(new CustomEvent('imweb:coupon:downloaded', { detail: { codes: downloadedCouponCodes } }));
          }
          renderTotalCouponWrap(totalCouponWrap, totalCouponDownloadButton, is_partial_fail);
    ...
    if (code != '') {
      $.ajax({
        ...
        success: function(res){
          ...
          if (res.download === "SUCCESS"){
            window.dispatchEvent(new CustomEvent('imweb:coupon:downloaded', { detail: { codes: [code] } }));
            ...
            if (detail_downloadable_coupon_count == 0 && res.msg) {
              const totalCouponWrap = document.querySelector('.total_coupon_wrap');
              if (totalCouponWrap) renderTotalCouponWrap(totalCouponWrap, res.msg, false);
            }
          } else {
