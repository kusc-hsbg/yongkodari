var SITE_SHOP_MYPAGE = function(){
      ...
      success : function(result){
        ...
        if(result.msg == 'SUCCESS'){
          if(result.count > 0){
            ...
            observeAlphaReviewInjection($order_list);
            ...
            $order_list.hide();
            if (alphaReviewObserver) {
              alphaReviewObserver.disconnect();
              alphaReviewObserver = null;
            }
            if(type === 'cancel'){
  ...
  var resizeBtnSize = function(list){
    ...
    if(window_width > 768){
      ...
      $btn_tool_row.width(Math.ceil(max_width));
    }
  };

  var alphaReviewObserver = null;

  var observeAlphaReviewInjection = function(list){
    if (typeof MutationObserver === 'undefined') return;
    if (alphaReviewObserver) {
      alphaReviewObserver.disconnect();
      alphaReviewObserver = null;
    }
    var nodes = list[0] ? list[0].querySelectorAll('.alpha_add_review_button') : [];
    if (!nodes.length) return;
    var scheduled = false;
    alphaReviewObserver = new MutationObserver(function(){
      if (scheduled) return;
      scheduled = true;
    ...
    for (var i = 0; i < nodes.length; i++) {
      alphaReviewObserver.observe(nodes[i], { childList: true, subtree: true });
    }
  };
