var SNS = function(){
    ...
    // [IWIS-2692] 상품상세(commerce) 공유 모달 + 친구초대 캠페인 운영중이면 상단에 배너+공유버튼 애드온(§3.8.3-1).
    var fieCtx = window['__friend-invite-event__'];
    var fieAddon = (share_type === 'commerce' && fieCtx && fieCtx.campaignRunning)
      ? "<magnet-shell data-manifest-url='/_/fo-friend-invite-event-widget/index.magnet.json' data-mode='share-addon'><friend-invite-event-widget data-clay-theme='light-only'></friend-invite-event-widget></magnet-shell>"
      : '';
    const html = `
