var HEADER_MEGA_DROPDOWN = function(){
  ...
  var is_header_config = false; // 디자인모드 상단설정 여부 true, false
	var close_pending = false;	// 여는 도중 예약됐던 close 를 열림 완료 후 재평가하기 위한 플래그
	var last_leave_e = null;	// 재평가 시 close 에 넘길 마지막 mouseleave 이벤트
	var isHovering = function(){
		return $widget.is(':hover') || $widget.find(':hover').length > 0
			|| $drop_down.is(':hover') || $drop_down.find(':hover').length > 0;
	};
		...
		$widget.find('._inline_menu_container').on('more_menu_complete',function(){
			...
			$widget.add($drop_down).off('mouseleave.mega_dropdown_' + code).on('mouseleave.mega_dropdown_' + code, function(e) {
				...
				close_timer = setTimeout(function() {
					// 여는 중이면 커서가 아직 펼쳐지는 패널 밖(아래쪽)에 있을 수 있어(sweep) mouseleave 가 오탐일 수 있다.
					// 이때 close 하면 slideUp 이 시작되고 곧 패널이 커서에 닿아 open 이 stop(true,true) 로 가로채 상단이 떨린다.
					// 그래서 close 판정을 '열림 완료' 시점으로 미룬다. 그 시점엔 패널이 full 크기라 :hover 판정이 정확하다. (IMOPS-12866)
					if(dropdown_state === 'opening'){
						close_pending = true;
						return;
					}
					if(isHovering()){
	...
	var open = function(e){
		...
		// 이미 열려있거나 여는 중이면 재진입 no-op. 여는 도중 mouseenter 가 stop(true,true) 로 full 높이까지 순간 점프하는 것을 막는다. (IMOPS-12866)
		if(dropdown_state === 'opening' || dropdown_state === 'open')
			return;
		...
		$drop_down.slideDown({
			...
			'done' : function(animation,jumpedToEnd){
				dropdown_state = 'open';
				// 열리는 도중 예약됐던 close 를 지금 재평가한다. 패널이 full 크기라 :hover 판정이 정확하다.
				if(close_pending){
					close_pending = false;
					if(!isHovering())
						close(last_leave_e);
	...
	var close = function(e){
		...
		// 이미 닫혀있거나 닫히는 중이면 재진입 no-op (open 과 대칭). (IMOPS-12866)
		if(dropdown_state === 'closing' || dropdown_state === 'closed')
			return;
