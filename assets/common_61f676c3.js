...
$.fn.setUploadImage = function(option, callback){
	...
	var option = $.extend(default_option, option);
	// 파일 선택 시 필터 결과를 업로드 대상에 반영하고 기존 change 핸들러를 이어서 실행한다.
	var original_change = option.change;
	if(typeof option.filterFiles === 'function'){
		option.change = function(e, data){
			var files = Array.prototype.slice.call(data.files || []);
			// 비지원 형식이 있으면 기존 add 오류 처리가 먼저 실행된다.
			if(files.every(function(file){
				return option.acceptList.includes(file.type);
			})){
				var filtered_files = option.filterFiles(files);
				if(!Array.isArray(filtered_files)){
					return false;
				}
			...
			if(typeof original_change === 'function'){
				return original_change.call(this, e, data);
			}
		};
	}
...
var setFroala = function(selector, option, events_option){
  ...
  if(!($image_upload_input_obj.length > 0)){
    ...
    $image_upload_input_obj.setUploadImage({
      ...
      filterFiles : option.image_upload_filter
    ...
    $image_replace_input_obj.setUploadImage({
      ...
      if (msg == 'success') {
        if(data_list.post_images.length > 0){
          // 저장된 이미지 객체 가져와서 교체
          var current_image = $image_replace_input_obj.data('current_image');
          if(current_image && current_image.length > 0) {
            ...
            // 사용 후 항상 정리
            $image_replace_input_obj.removeData('current_image');
          }
        }else{
          alert(msg);
        }
  ...
  FroalaEditor.RegisterCommand(image_replace, {
    ...
    refreshAfterCallback: false,
    callback: function () {
      // 현재 선택된 이미지를 이 에디터의 교체 input에 저장
      var $current_img = this.image.get();
      if($current_img && $current_img.length > 0) {
        $image_replace_input_obj.data('current_image', $current_img);
      }
      // 파일 선택 다이얼로그 열기
      $image_replace_input_obj.find("._image_replace_btn").click();
    }
  });
  ...
  var default_events_option = {
    ...
    'image.beforeUpload': function(images, image){
      if(!use_image_upload){
        alert(getLocalizeString('설명_드래그업로드는지원하지않습니다', '', '드래그 업로드는 지원하지 않습니다.'));
        //cancelPropagation(e);
        return false;
      }
      // Froala가 실제 전송하는 첫 이미지에 파일 필터를 적용하고, 거절되면 업로드를 중단한다.
      if(typeof option.image_upload_filter === 'function'){
        var upload_image = images && images.length > 0 ? images[0] : null;
        var upload_image_type = upload_image ? upload_image.type.replace(/image\//g, '') : '';
        // 비지원 형식은 Froala의 기존 오류 처리에 맡긴다.
        if(upload_image && this.opts.imageAllowedTypes.includes(upload_image_type)){
          var valid_images = option.image_upload_filter([upload_image]);
          if(!Array.isArray(valid_images) || valid_images.length === 0){
            if(image && typeof image.attr === 'function' && typeof image.remove === 'function'){
              var image_src = image.attr('src') || '';
              if(image_src.indexOf('data:') === 0 || image_src.indexOf('blob:') === 0){
                image.remove();
              }
            }
            // Froala DnD는 image 인자 없이 image.insert popup을 연 뒤 이 이벤트를 호출하므로,
            // 업로드 취소 후 popup이 남지 않도록 닫는다.
            if(!image){
    ...
    // 연관 상품 저장소 div(#shoppable-additional-related-product-list) 보호.
    // contenteditable="false"는 일반 커서 편집만 막을 뿐, 코드뷰(</>) 소스 직접 편집이나
    // Ctrl+A 후 전체 교체로는 div 자체가 통째로 삭제된다. 삭제되면 data-shoppable-tags(상품코드 원천)가
    // 유실되어 연동이 끊기므로, 코드뷰 진입 직전 원본 outerHTML을 스냅샷해 두고
    // 코드뷰 커밋(html.set) 후 div가 사라졌으면 스냅샷을 그대로(조용히) 복원한다.
    'commands.before': function(cmd){
      // 코드뷰 토글(html) 버튼을 누르기 직전, 살아있는 원본 div를 스냅샷으로 붙잡는다.
      if(cmd === 'html'){
        var $keep = this.$el.find('#shoppable-additional-related-product-list');
        if($keep.length){
          this._shoppableProtectSnapshot = $keep.first().get(0).outerHTML;
        }
      }
    },
    'html.set': function(){
      // html.set은 최초 로드/paste/코드뷰 커밋 등 본문 innerHTML이 교체되는 공통 관문이다.
      // 코드뷰 커밋(T 함수)은 c.html.set(소스) 실행 후 이 이벤트를 발동하므로,
      // 이 시점에는 이미 새 DOM이 반영돼 있어 "사라졌는지"를 정확히 판정할 수 있다.
      var $editor = this.$el;
      var $exist = $editor.find('#shoppable-additional-related-product-list');
      if($exist.length){
        // 아직 남아 있으면 최신 상태를 스냅샷으로 갱신(다음 편집 사이클 대비).
        this._shoppableProtectSnapshot = $exist.first().get(0).outerHTML;
        return;
      }
      // div가 사라졌는데 복원할 스냅샷이 있으면: 알림 없이 원본 그대로 재삽입.
      if(this._shoppableProtectSnapshot && !this._shoppableProtectRestoring){
        this._shoppableProtectRestoring = true;
        try {
          this.$el.append(this._shoppableProtectSnapshot);
        } finally {
          this._shoppableProtectRestoring = false;
        }
      }
    },
  };
